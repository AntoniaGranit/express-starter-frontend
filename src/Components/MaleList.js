...
export const MaleList = () => {
  ...
  useEffect(() => {
    setLoading(true)
    fetch('https://project-express-api-wcsanbxhyq-lz.a.run.app/professionals/male')
      .then((res) => res.json())
      .then((data) => setMaleList(data.body.professionals))
  ...
  return (
    <section className="completelist-container">
      {maleList.map((singleProfessional) => {
        return (
