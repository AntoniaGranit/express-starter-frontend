...
export const FemaleList = () => {
  ...
  useEffect(() => {
    setLoading(true)
    fetch('https://project-express-api-wcsanbxhyq-lz.a.run.app/professionals/female')
      .then((res) => res.json())
      .then((data) => setFemaleList(data.body.professionals))
  ...
  return (
    <section className="completelist-container">
      {femaleList.map((singleProfessional) => {
        return (
